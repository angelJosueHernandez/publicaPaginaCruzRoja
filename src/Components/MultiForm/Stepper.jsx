          ...
          className={index!= newStep.length -1 ?  'w-full flex items-center': 'flex items-center'}>
              <div className='relative flex flex-col items-center text-teal-600'>
                  <div className={`rounded-full transition duration-500 ease-in-out border-2
                      border-red-300 h-12 w-12 flex items-center justify-center py-3 ${step.selected ? 'bg-red-600 text-white font-bold border border-red-600':''}`}>
                      {/*numero */}
                      {step.completed ? (
                  ...
                  <div className={`flex-auto border-t-2 transition duration-500 ease-in-out ${step.completed ? 'border-red-600' : 'border-red-300'}`}>
                      {/*linea*/}
                  </div>
          </div>
      );
