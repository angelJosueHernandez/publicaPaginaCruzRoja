...
const handleClick = async (direction) => {
  ...
  fetch(`https://api-git-main-ah5318740gmailcoms-projects.vercel.app/recuperacionCorreo/${encodeURIComponent(correo)}`, {
    method: "POST",
    headers: {
    ...
    setTimeout(() => {
      message.error('Error al enviar el Token. Por favor, intenta de nuevo más tarde.');
        ...
        if(correo == null){
          ...
          message.loading('Verficando...', 1);
            fetch('https://api-git-main-ah5318740gmailcoms-projects.vercel.app/recuperacionSMS', {
              method: "POST",
              headers: {
                "Content-Type": "application/json"
              },
  ...
  if (currentStep === 3 && direction === 'Siguiente') {
    if (contraActualizar === null && contraActualizar2 === null) {
      ...
      if(contraCo === false && contraCo2 === false){
        ...
        fetch(`https://api-git-main-ah5318740gmailcoms-projects.vercel.app/actualizarContraRecuperacion`, {
          method: "PUT",
          headers: {
            "Content-Type": "application/json"
          },
...
const verifyToken = async () => {
  if (token == null) {
    ...
    try {
      const response = await fetch(`https://api-git-main-ah5318740gmailcoms-projects.vercel.app/compararToken/${encodeURIComponent(correo)}`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json"
        },
